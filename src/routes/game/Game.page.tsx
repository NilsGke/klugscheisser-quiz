...
const Game = ({
  ...
  const grantPoints = (points: number) => {
    ...
    setGameData((prev) => {
      ...
      const field =
        newCategories[selected.categoryIndex].fields[selected.fieldIndex];
      ...
      <div id="gamePage">
        <Setup
          setGameData={(gameData: Game) => {
            ...
            if (settings.theme === "senior") setGameState(State.intro);
          }}
        />
      </div>
    );
    ...
    <div id="gamePage" className={gameState}>
      <aside>
          ...
          {gameData.teams.map((team, teamIndex) => (
            <Team
              ...
              setTeam={(newTeam: GameTeam) => {
                const newTeams = gameData.teams.slice() as Game["teams"];
        ...
        <div id="gameControls" ref={gameControlsRef}>
          ...
          <button className="settings" onClick={() => setSettingsOpen(true)}>
            <img src={gearIcon} alt="settings icon" title="settings" />
          </button>
          ...
          {testMode ? (
            ...
            >
              <img src={finishFlagIcon} alt="finish icon" title="end game" />
            </button>
          )}
            ...
            <>
              {category.fields.map((field, fieldIndex) => (
                <Field
                  ...
                  grantPoints={(points) => grantPoints(points)}
                  selected={
                  ...
                  onClick={() => {
                    abortTimers();
                    startGameSequence(categoryIndex, fieldIndex);
                  }}
                  setGameState={(newState: State) => setGameState(newState)}
      ...
      {gameState === State.done ? (
        <div className="doneWrapper">
          <div className="done">
            <button className="close" onClick={() => setGameState(State.idle)}>
            ...
            <div className="winners">
              {sortedTeams[1] !== undefined ? (
                <div className="team second">
                  ...
                  <div className="points">{sortedTeams[1].score}</div>
                </div>
              ) : null}
              ...
              {sortedTeams[0] !== undefined ? (
                <div className="team first">
                  ...
                  <div className="points">{sortedTeams[0].score}</div>
                </div>
              ) : null}
              ...
              {sortedTeams[2] !== undefined ? (
                <div className="team third">
                  ...
                  <div className="points">{sortedTeams[2].score}</div>
                </div>
              ) : null}
            </div>
                          ...
                          <button
                            ...
                            onClick={() =>
                              confirmAlert({
                                ...
                                childrenElement: () =>
                                  answeredFields.map((field, fieldIndex) => {
                                    return (
                                      <>
                                        <div
                                          key={fieldIndex}
                                ...
                                overlayClassName: "answeredQuestions",
                                closeOnEscape: true,
...
const Field = ({
  ...
  useEffect(() => {
    ...
    if (containerRef.current === null) throw new Error("containerRef is null");
  ...
  return (
    <div className="fieldContainer" ref={fieldContainerRef}>
      <div
        ...
        onClick={
          typeof field.answered !== "string" && !selected ? onClick : undefined
        }
        ref={fieldRef}
        style={{
          borderColor: selected && timeUp ? "#ff6464" : undefined,
          boxShadow: selected && timeUp ? "0 0 30px #ff6464" : undefined,
        }}
      >
            ...
            {selected ? (
              <>
                <div
                  ...
                  style={{
                    opacity: gameState === State.showDescription ? 1 : 0,
                  }}
                >
                  ...
                  settings.descriptionNext === "auto" ? (
                    ...
                    <button
                      className="next"
                      onClick={() => setGameState(State.showQuestion)}
                    >
                      next
                    </button>
                  )}
                ...
                {gameState === State.showQuestion ? (
                  <div
                    className="question"
                    style={{
                      opacity: gameState === State.showQuestion ? 1 : 0,
                    }}
                  >
                    ...
                    <div className="bottom">
                      {buzzeredTeamIndex !== null && settings.useAnswerTime ? (
                        <TimeBar time={category.answerTime * 1000} reverse />
                      ) : null}
                      ...
                      <button
                        ...
                        style={{
                          opacity:
                            buzzeredTeamIndex === null && !testMode ? 0 : 1,
                        }}
                        onClick={() => setGameState(State.showAnswer)}
                      >
                        show Answer
                      </button>
                    </div>
                ...
                {gameState === State.showAnswer ? (
                  <div
                    className="answer"
                    style={{
                      opacity: gameState === State.showAnswer ? 1 : 0,
                    }}
                  >
                    <ResourceDisplay
                      settings={settings}
                    ...
                    {buzzeredTeamIndex !== null ? (
                      <div className="points">
                        <div className="normal">
                          ...
                          <button
                            onClick={() => setShowCustomPoints((prev) => !prev)}
                          >
                        ...
                        <div
                          className={
                            "custom" +
                            (showCustomPoints ? " visible" : " hidden")
                          }
                        >
                          <button
                            onClick={() => {
                              grantPoints(Math.round(points / 2) * -1);
                              close();
                            }}
                          >
                            -{Math.round(points / 2)}
                          ...
                          <input
                            type="number"
                            placeholder={Math.round(points / 3).toString()}
                            value={customPoints}
                            onChange={(e) =>
                              setCustomPoints(
                          ...
                          <button
                            onClick={() => {
                              grantPoints(Math.round(points / 2));
                              close();
                            }}
                          >
                            {Math.round(points / 2)}
  ...
  return (
    <div
      ...
      style={{
        ...
        background:
          theme === "senior" && (buzzeredTeamIndex === null || buzzered)
            ? team.color
            : undefined,
      }}
      ref={teamRef}
