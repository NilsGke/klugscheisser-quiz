...
const Edit = ({
  ...
  const saveCategory = async () => {
    ...
    if (
      !category.fields.every(
        (field) => field.question !== undefined && field.answer !== undefined,
      )
    )
    ...
    if (!isCategory(category)) return toast("❌your category is not complete!");

    // save category
    let promiseDbIndex: Promise<Indexed<Category>["dbIndex"]>;
    if (dbIndex)
      promiseDbIndex = updateCategoryInDB(indexCategory(category, dbIndex));
  ...
  return (
    <div id="edit">
      <BackButton to="/categories" confirm={unsavedChanges ?? undefined} />
            ...
            <>
              ...
              <label className="image">
                ...
                <input
                  ...
                  onInput={() => {
                    if (descriptionImageInputRef.current === null) return;
                    const files = descriptionImageInputRef.current.files;
            ...
            addResource
              ? (index, fieldType) => {
                  ...
                  if (addResource.type === "image") {
                    if (
                      newCategory.fields[index][fieldType]?.type === "image" &&
                      (newCategory.fields[index][fieldType]?.content as Image)
                      ...
                      newCategory.fields[index][fieldType]?.type ===
                        "imageCollection" &&
                      (
                        newCategory.fields[index][fieldType]?.content as File[]
                      ).find((img) => img.name === addResource.content.name) ===
                        undefined
                    )
                      newCategory.fields[index][fieldType] = {
                        type: "imageCollection",
                      ...
                      newCategory.fields[index][fieldType] = addResource;
                    }
                  } else {
                    newCategory.fields[index][fieldType] = addResource;
                  }
