...
const MediaPool = ({
  ...
  useEffect(() => {
    if (loadMore) {
      getStoredFiles(mediaType, (files.at(-1)?.dbIndex || 999999999999) - 1, 10)
    ...
    <div className="mediaPool" ref={listRef}>
      ...
      <div className="header">
        ...
        <div className="storage">
          <div className="info">{humanFileSize(storageSize, true)}</div>
          <button
            className="clear"
            onClick={() =>
              confirm(
              ...
              onChange={async (e) => {
                ...
                if (files !== null && files.length !== 0)
                  for (let i = 0; i < files.length; i++) {
                    const file = files[i];
                    await storeFileInIndexedDB(file, mediaType).then(
                      async (dbIndex) => {
                        const indexedFile = file as IndexedFile;
                        indexedFile.dbIndex = dbIndex;
                        setFiles((prev) => [indexedFile, ...prev]);
          ...
          <File
            ...
            deleteFile={() =>
              deleteStoredFile(mediaType, file.dbIndex).then(() => {
                setFiles((prev) =>
                  prev.filter((f) => f.dbIndex !== file.dbIndex),
                );
                setLength((prev) => prev - 1);
                checkScroll();
              })
...
const File = ({
  ...
  return (
    ...
    >
      {MediaElement}
      <button className="dots" onClick={() => setPopoverOpen((prev) => !prev)}>
      ...
      {popoverOpen ? (
        <Popover
          ...
          view={() => {
            ...
            if (newWindow) newWindow.onload = () => URL.revokeObjectURL(url);
          }}
          rename={
            mediaType === "audio"
              ? () => {
