  ...
  useEffect(() => {
    getThing<typeof mapping>("gamepadMapping")
      .then((mapping) => {
        ...
        if (typeof mapping[0] === "number") throw Error("old mapping type");

        console.log({ mapping, m: mapping.length, t: teams.length });

        if (mapping.length < teams.length)
          mapping.push(...new Array(teams.length - mapping.length).fill([]));
      ...
      {overlayVisible ? (
        <div className="gamepadOverlayWrapper">
          <div className="gamepadOverlay">
            <button className="close" onClick={() => setOverlayVisible(false)}>
              <img src={closeIcon} alt="close" />
            </button>
            <Mapping mapping={mapping} teams={teams} update={setMapping} />
          </div>
        </div>
      ) : null}
    </>
            ...
            return (
              <React.Fragment key={teamIndex}>
                ...
                <ButtonList
                  ...
                  adding={
                    addingButton !== false && addingButton.team === teamIndex
                  }
                  startAdding={() =>
                    setAddingButton({
                      team: teamIndex,
  ...
  return (
    <div className="buttons" ref={buttonListRef}>
      {keyIdArray.map((keyId, keyIndex) => (
        <Button keyId={keyId} onClick={() => remove(keyIndex)} key={keyId} />
      ))}
      {adding ? (
        <button
          key={"add"}
...
const Button = ({ keyId, onClick }: { keyId: string; onClick: () => void }) => {
  return (
    <button className={"button"} title="remove this button" onClick={onClick}>
