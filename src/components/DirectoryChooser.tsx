      ...
      new Promise<FSDHWithKey[]>(async (resolve, reject) => {
        const [keys, FSDHs] = await Promise.all([
          ...
          new Promise<FSDH[]>((resolve, reject) => {
            ...
            request.onsuccess = () => resolve(request.result as FSDHWithKey[]);
            request.onerror = () => reject(request.error);
          }),
        ]);
  ...
  if (needPermission && fsdh)
    return (
      <div id="directoryChooser">
        <h2>
          Bitte gib der Webseite die benötigten Berechtigungen für Ordner: "
          {fsdh.name}"
        </h2>
        <button
          onClick={() =>
            checkPermissions().then(() => setFileSystemDirectoryHandle(fsdh))
          }
        >
    ...
    <div id="directoryChooser">
      {!supported && (
        <>
          ...
          Bitte benutze Chrome, Edge oder Opera (oder einen Chromium browser){" "}
          <br />
          <a
              ...
              window
                ...
                .catch((err) => {
                  if (err.name === "AbortError")
                    return toast("❌ Du hast kein Verzeichnis ausgewählt");
              ...
              {handles
                ...
                .map((handle) => (
                  <button
                    ...
                    onClick={() => {
                      setFsdh(handle.fileSystemDirectoryHandle);
                      if (db)
                        updateFSDHLastOpened(db, handle.key).then(() =>
                          refetchRecent(),
                        );
                    }}
                  >
                    ...
                    <p>
                      <RelativeTime time={handle.lastOpened} />
                    </p>
                  </button>
                ))}
            </div>
