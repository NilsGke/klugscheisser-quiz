...
const VideoPlayer: FC<props> = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      videoElementRef.current?.removeEventListener("timeupdate", timeUpdate);
    };
  }, [videoElementRef]);
  ...
  useEffect(() => {
    if (videoElementRef.current) videoElementRef.current.volume = volume / 100;
  }, [volume]);
  ...
  const changeTime = (currentTime: number) => {
    if (videoElementRef.current === null)
      throw new Error("audio element ref is null -> could not change time");
    ...
    >
      <div className="volume">
        <Slider
          ...
          max={100}
          onChange={
            (value) => setVolume(typeof value === "object" ? value[0] : value) // not pretty and technically unecessery but true typesafety
          }
          vertical={!small}
        />
        <button
      ...
      <div className="videoContainer">
        ...
        <button className="playPause" onClick={() => play(!playing)}>
          <img src={playing ? pauseIcon : playIcon} draggable="false" alt="" />
        </button>
        <button
