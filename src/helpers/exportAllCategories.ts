...
const exportAllCategories = (
  ...
  new Promise<void>(async (resolve, reject) => {
    ...
    const generate = (zip: JSZip) =>
      zip
        .generateAsync({ type: "blob" }, (info) =>
          update({
            currentAction: `packing zip file (${count}): ` + info.currentFile,
        ...
        .then((file) =>
          downloadFile(
            file,
            `backup_${new Date().toLocaleDateString(navigator.language, {
              year: "numeric",
              month: "numeric",
              day: "numeric",
            })}_${count}.ksq.zip`,
    ...
    for (let i = 0; i < zippedNames.length; i++) {
      ...
      zip.file(`${zipped.categoryName}_${zipped.dbIndex}.ksq.zip`, zipped.file);
