    ...
    JSZip.loadAsync(file).then(async (zip) => {
      ...
      if (config.description.type === "text") {
        ...
        const mediaBlob = await zip
          ...
          ?.async("blob");
        if (mediaBlob === undefined) throw new Error("media blob is undefined");
...
const applyMediaFromZipFile = (
  ...
  new Promise<void>(async (resolve, reject) => {
    if (configResource.type === "text")
      category.fields[fieldIndex][fieldType] = configResource as TextResource;
    else {
      // image collection
      if (
        ...
        await Promise.all(
          configResource.content.map(async (fileName) => {
            ...
            (category.fields[fieldIndex][fieldType]?.content as Image[]).push(
              file as Image,
            );
          }),
        );
